...
usage(){ echo;echo -e "${col_wt}${col_dgbg}Usage:${col_r}"
...
################" SET IMAGE SIZE    echo -e " \"$name -s\"    ${col_ct}set image size${col_r}"
## 1920x1200
## 1920x1080
## 1366x768
## 800x600
...
helptxt(){ echo;echo -e "${col_lot} §${col_r} bingwallpaper.sh is a simple script that retrieve 'Bing image of the day' and"
...
echo -e "   run it as a daemon (${col_wt}${col_dgbg}-d${col_r}), or add it to cron jobs (${col_wt}${col_dgbg}-c${col_r}).";echo
...
echo "   yet, it will be created if possible.";echo
...
testapi(){ attempt=1;while [ $attempt -le 3 ]
do apiResp=$(curl -s $reqImg);if [ $? -gt 0 ];then
echo -e -n "${col_lrt}API not responding.${col_r} Retry $attempt/3 in 10s"
...
echo "(to run bingwallpaper as daemon, please remove cron job first: bingwallpaper.sh -r)"
echo;exit;fi
...
echo "(to add bingwallpaper to cron jobs, please stop the daemon first: bingwallpaper.sh -k)"
echo;exit
else
run_once=true
...
;;
-i)
echo "Current check interval: $checkint hours";echo $sep
while true; do
read -p "Enter new interval (in hours) - q pour quitter : " new_checkint
...
;;
-f)
echo "Current bing wallpapers folder: $wpath";echo $sep
while true; do
read -p "Enter a new folder (q to quit): " selected_path
...
;;
-h|-\?)
echo -e "$titl";helptxt;usage;exit
;;
-p)
echo -e "$titl"
...
#test if image is valid (contains .jpg)
...
/opt/trinity/bin/dcop --no-user-time --user $USR --all-sessions kdesktop KBackgroundIface setWallpaper "$wpath$imgName" 6
fi
if [ "$run_once" == true ] && [ "$incron" -eq 0 ];then
